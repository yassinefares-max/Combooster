...
def generate_complete_calendar():
    """Génère un calendrier complet avec images et ad-copies automatiques"""
    try:
        data = request.get_json()
        site_id = data.get("site_id", "").strip()
        ...
        # Récupérer le profil client - avec conversion des ObjectId
        site_info = rag_system.profile_manager.get_profile(site_id)
        if site_info:
            site_info = clean_mongo_data(site_info)
        
        # Convertir ObjectId en string pour la sérialisation JSON
        if site_info and '_id' in site_info:
            site_info['_id'] = str(site_info['_id'])
        
        print(f"🎯 Génération calendrier complet pour {site_info.get('company_name', 'Client')}")
        
        # Générer la stratégie de contenu avec Gemini
        strategy_prompt = f"""
En tant qu'expert en stratégie de contenu, planifie un calendrier éditorial complet pour {duration_weeks} semaines avec {posts_per_week} posts par semaine.

PROFIL CLIENT:
- Entreprise: {site_info.get('company_name', 'Inconnu')}
- Industrie: {site_info.get('industry', 'Inconnu')}
- Type: {site_info.get('business_type', 'Inconnu')}
- Positionnement: {site_info.get('market_position', 'Standard')}
- Voice: {site_info.get('brand_voice', 'Professionnel')}
- Audience: {site_info.get('target_audience', 'Général')}

Génère un plan détaillé avec pour CHAQUE post:
1. Thème du post
2. Type de contenu (education, promotion, inspiration, engagement)
3. Angle créatif
4. Objectif marketing  
5. Idée visuelle
6. Moment optimal (9:00, 12:00, 15:00, 18:00, 21:00)
        ...
        {{
            ...
            "days": [
                {{
                    "day": "Lundi",
                    "post_number": 1,
                    "theme": "Sous-thème spécifique",
                    "content_type": "education",
                    "creative_angle": "Angle créatif unique",
                    "marketing_goal": "Objectif mesurable",
                    "visual_idea": "Description détaillée de l'image",
                    "best_time": "12:00"
                }}
            ]
        }}
...
Crée un contenu varié et engageant adapté à l'industrie {site_info.get('industry', '')}.
"""

        strategy_response = rag_system.generate_response(strategy_prompt)
        print(f"📋 Réponse stratégie Gemini: {strategy_response[:500]}...")
        ...
        try:
            ...
            if json_match:
                ...
                print(f"✅ Stratégie parsée: {len(calendar_strategy.get('weeks', []))} semaines")
            else:
                # Fallback: créer une stratégie basique
                print("⚠️ Fallback stratégie basique")
                calendar_strategy = create_fallback_strategy(duration_weeks, posts_per_week, site_info)
        except Exception as e:
            print(f"❌ Erreur parsing stratégie: {e}")
            calendar_strategy = create_fallback_strategy(duration_weeks, posts_per_week, site_info)
        
        # Générer les images et ad-copies pour chaque post
        calendar_with_content = {
            "strategy": calendar_strategy,
            "generated_posts": [],
            "company_info": {
                "company_name": site_info.get('company_name', 'Client'),
                "industry": site_info.get('industry', 'Général'),
                "brand_voice": site_info.get('brand_voice', 'Professionnel')
            }
        }
        ...
        # Parcourir toutes les semaines et posts
        for week in calendar_strategy.get("weeks", []):
            week_number = week.get("week_number", 1)
            for day in week.get("days", []):
                if generated_count >= total_posts:
                    break
                    
                try:
                    post_number = day.get("post_number", generated_count + 1)
                    theme = day.get("theme", f"Post {post_number}")
                    content_type = day.get("content_type", "general")
                    visual_idea = day.get("visual_idea", f"Image pour {theme}")
                    
                    print(f"🔄 Génération post {generated_count + 1}/{total_posts}: {theme}")
                    
                    # Générer le prompt d'image
                    image_prompt = generate_automated_image_prompt_internal(
                        site_id, 
                        f"{theme} - {visual_idea} - Type: {content_type}",
                        site_info
                    )
                    
                    if image_prompt:
                        # Générer l'ad-copy
                        ad_copy = generate_contextual_ad_copy_internal(
                            site_id,
                            image_prompt,
                            content_type,
                            site_info
                        ...
                        # Générer l'image avec Leonardo
                        generated_image = None
                        if LEONARDO_API_KEY and image_prompt:
                            try:
                                from leonardo_ai import LeonardoAIGenerator
                                leonardo = LeonardoAIGenerator(LEONARDO_API_KEY)
                                
                                image_result = leonardo.generate_without_reference(
                                    ad_copy=image_prompt,
                                    tone=site_info.get('brand_voice', 'professional'),
                                    client_context={
                                        'industry': site_info.get('industry', ''),
                                        'brand_voice': site_info.get('brand_voice', ''),
                                        'brand_values': site_info.get('brand_values', [])
                                ...
                                if image_result.get("success") and image_result.get("images"):
                                    ...
                                    print(f"✅ Image générée pour post {post_number}")
                            except Exception as e:
                        ...
                        post_data = {
                            ...
                            "best_time": day.get("best_time", "12:00"),
                            "creative_angle": day.get("creative_angle", ""),
                            "marketing_goal": day.get("marketing_goal", "")
                        }
                        
                        calendar_with_content["generated_posts"].append(post_data)
                        generated_count += 1
                        
                    else:
                        print(f"❌ Échec génération prompt image pour post {post_number}")
                        
                except Exception as e:
                    print(f"❌ Erreur génération post {generated_count + 1}: {e}")
                    continue
        
        print(f"✅ Calendrier généré: {generated_count}/{total_posts} posts créés")
        ...
        return jsonify({
            ...
            "stats": {
                ...
                "total_posts_generated": generated_count,
                "duration_weeks": duration_weeks,
                "posts_per_week": posts_per_week,
                "success_rate": f"{(generated_count/total_posts)*100:.1f}%"
            }
        })
        
    except Exception as e:
        print(f"❌ Erreur génération calendrier complet: {e}")
        import traceback
        print(f"🔍 Détails: {traceback.format_exc()}")
        return jsonify({"success": False, "error": str(e)})
